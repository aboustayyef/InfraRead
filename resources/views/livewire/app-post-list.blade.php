<div  class="pt-12 relative text-left w-full h-screen overflow-y-auto p-12">
    ...
    @foreach($posts as $key => $post)
        <div class="max-w-7xl mx-auto cursor-pointer mb-6 border-b border-gray-200">
            <div wire:click="$emit('postSelected',{{$post->id}})" class="flex">
                <div class="mr-12 w-1/2">
                    ...
                    <h4 class="mt-4 text-gray-500 text-md">{{$post->posted_at->diffForHumans()}}</h4>
                </div>
                <div class="w-1/2 leading-relaxed text-gray-400 text-xl">
                    <p>{{$post->excerpt}}</p>
                </div>
            </div>
